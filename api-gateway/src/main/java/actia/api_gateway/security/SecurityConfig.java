...
public class SecurityConfig {
     ...
     *
     * Comportement :
     * - Si l'utilisateur a déjà une session valide → redirect vers le frontend (dashboard)
     * - Si pas de session → redirect vers Keycloak pour un nouveau login propre
     *
     * Sans ce filtre : Spring redirige vers /login?error → 404 Whitelabel Error Page.
     */
    @Bean
            ...
            return exchange.getSession()
                    .flatMap(session -> {
                        ...
                        URI target = hasSession
                                // Session active → renvoie directement au dashboard Angular
                                ? frontendUrl
                                // Pas de session → nouveau login Keycloak propre
    ...
    /**
     * À chaque requête authentifiée : stocke le id_token dans la session Redis.
     * Nécessaire pour passer id_token_hint à Keycloak au logout
     * → évite la page de confirmation "Do you want to log out?".
     */
    @Bean
    ...
    /**
     * Intercepte GET /logout AVANT Spring Security.
     * Invalide la session Redis + efface le cookie SESSION
     * + redirige vers Keycloak end_session avec id_token_hint (logout silencieux).
     */
    @Bean
